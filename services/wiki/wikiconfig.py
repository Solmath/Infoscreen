...
class Config(DefaultConfig):
    ...
    try:
        interwiki_map = InterWikiMap.from_file(
            os.path.join(wikiconfig_dir, "intermap.txt")
        ).iwmap
    ...
    registration_only_by_superuser = (
        True  # True disables self-registration, recommended for public wikis
    )
    ...
    acl_functions = "Thomas:superuser"
    ...
    acls = {
        ...
        NAMESPACE_DEFAULT: dict(
            before="Thomas:read,write,create,destroy,admin",
    ...
    namespace_mapping, backend_mapping, acl_mapping = create_mapping(
        uri,
        namespaces,
        backends,
        acls,
